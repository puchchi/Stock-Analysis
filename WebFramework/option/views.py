...
def totalCallPut(request, arg):
    ...
    tableName = "HistoricOption" + arg[0] + arg[1] + arg[2]
    ceResult = []
    peResult = []
    try:
...
############################# AR Views ########################################
#def arView(request):
#    return render(request, 'option/argonar.html')

def pktrial(request, value):
    imageSrc = "option/pktrial" + str(value) + ".png"
    return render(request, 'option/pktrial.html', {'imageSrc':imageSrc})

def arIndex(request):
    return render(request, 'option/ar_index.html')

def arVideoPlayer(request, path):
    print path
    return render(request, 'option/ar_video_player.html', {'videoName':'option/'+path+".mp4"})

def arView2(request):
    return render(request, 'option/argonar.html')

def indesignARView(request):
    return render(request, 'option/IndesignAR.html')

def ar3(request):
    return render(request, 'option/ar3.html')

def ar4(request):
    return render(request, 'option/ar4.html')

def indesignVRView(request):
    return render(request, 'option/VRDemo.html')

def imageColorEditing(request):
    return render(request, 'option/image_color_editing.html')