...
from scraper.spiders import LiveOptionSpider
...
# Use kLiveOption for single time scraping
class kLiveOption:
    def __init__(self, stockName, expiries, instrumentType, databaseTableName):
    ...
    def __call__(self):
        ...
        process.crawl(LiveOptionSpider.kLiveOptionSpider ,symbol=self.stockName, expiries=self.expiries, instrumentType=self.instrumentType,
...
class kCommand:
    ...
    def run_spider(self, queue, args):
        try:
            ...
            deferred = runner.crawl(LiveOptionSpider.kLiveOptionSpider, symbol=args[0][0], expiries=args[0][1], instrumentType=args[0][2],
                                    databaseTableName=args[0][3])
    ...
    def get_name(self):
        return "Live Option Command"

if __name__ == "__main__":
    liveOption = kLiveOption("SBIN", ["May2018", "Jun2018"], "OPTSTK", "LiveOption")
    liveOption()

    cmd = kCommand(["SBIN", ["Jan2018", "Feb2018"], "OPTSTK", "LiveOption"])
    cmd.do()
