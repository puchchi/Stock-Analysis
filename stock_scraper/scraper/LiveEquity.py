...
from scraper.spiders import LiveEquitySpider
...
# Use kLiveEquity for single time scraping
class kLiveEquity:
    ...
    def __call__(self):
        ...
        process.crawl(LiveEquitySpider.kLiveEquitySpider ,symbol=self.symbol,  databaseTableName=self.databaseTableName)
...
class kCommand:
    ...
    def run_spider(self, queue, args):
        try:
            ...
            deferred = runner.crawl(LiveEquitySpider.kLiveEquitySpider, symbol=args[0][0], databaseTableName=args[0][1])
    ...
    def get_name(self):
        return "Live Equity Command"
if __name__ == "__main__":
    liveEquity = kLiveEquity("RELIANCE", "LiveStock")
    liveEquity()

    cmd = kCommand(["SBIN", "LiveStock"])
    #cmd.do()
